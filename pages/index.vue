<template>
    <div class="flex bg-zinc-900 h-screen">
        <!-- sidebar -->
        <div class="bg-black w-[338px] p-8 flex flex-col overflow-y-auto filter__checkbox-list filter__checkbox-list">
            <div>
                <Logo/>
            </div>
                ...
                <div class="ml-2 space-y-2">
                    ...
                    >
                            <h3 class="text-sm font-bold text-[#F4F4F5] truncate">
                                {{ note.name !== "Untitled" ? note.name : note.text.substring(0, 20) }}
                            </h3>
                            <div class="leading-none truncate text-[#D6D6D6]">
                            <span class="text-xs text-[#F4F4F5] mr-4">{{
                                new Date(note.updatedAt).toLocaleDateString()
                            }}</span>
                            <span v-if="note.text.length > 0 && note.name !== 'Untitled'" class="text-xs text-[#D6D6D6]"
                            >...{{ note.text.substring(0, 50) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                      ...
                      new Date().toDateString()
                          ...
                          : new Date(note.updatedAt).toLocaleDateString()
                            ...
                            <span v-if="note.text.length > 50"
                                  class="text-xs text-[#D6D6D6]">...{{ note.text.substring(50, 100) }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full flex flex-col">
            ...
            <div class="max-w-[437px] mx-auto w-full flex-grow flex flex-col">
                ...
                <textarea
                    ref="textarea"
                    maxlength="28"
                    class = "text-[#D4D4D4] font-playfair w-full bg-transparent focus:outline-none resize-none text-3xl whitespace-pre-wrap overflow-hidden h-10"
                    name="name"
                    id="name"
                    v-model="updatedName"
                    @input="() => {
                        debouncedFn()
                        selectedNote.name = updatedName
                    }"
                />
                <textarea
                    ref="textarea"
                    ...
                    name="note"
                    id="note"
                    class="text-[#D4D4D4] my-5 font-playfair w-full bg-transparent focus:outline-none resize-none flex-grow filter__checkbox-list filter__checkbox-list"
                    @input="() => {
                      debouncedFn()
...
function setNote(note) {
    ...
    updatedName.value = note.name
    const pageCookie = useCookie('NoteNestPage')
    pageCookie.value = note.id
}

function logout() {
    const jwtCookie = useCookie('NoteNestJWT')
    const pageCookie = useCookie('NoteNestPage')
    pageCookie.value = null
...
async function createNewNote() {
    try {
        ...
        updatedNote.value = ''
        updatedName.value = 'Untitled'
        textarea.value.focus()
        const pageCookie = useCookie('NoteNestPage')
        pageCookie.value = notes.value[0].id
    } catch (err) {
        console.log(err)
    }
}
...
onMounted(async () => {
    ...
    for (let i = 0; i < notes.value.length; i++) {
        if (notes.value[i].id === Number(useCookie('NoteNestPage').value)) {
            setNote(notes.value[i])
        }
    }
})
